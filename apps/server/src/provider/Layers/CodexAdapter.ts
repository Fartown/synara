...
const makeCodexAdapter = (options?: CodexAdapterLiveOptions) =>
  Effect.gen(function* () {
    ...
    const listExternalThreads: NonNullable<CodexAdapterShape["listExternalThreads"]> = (input) =>
      Effect.tryPromise({
        try: () => manager.listExternalThreads(input),
        catch: (cause) =>
          new ProviderAdapterRequestError({
            provider: PROVIDER,
            method: "thread/list",
    ...
    const archiveExternalThread: NonNullable<CodexAdapterShape["archiveExternalThread"]> = (
      input,
    ) =>
      Effect.tryPromise({
        try: () => manager.archiveExternalThread(input),
        catch: (cause) =>
          new ProviderAdapterRequestError({
            provider: PROVIDER,
            method: "thread/archive",
            detail: toMessage(cause, "Failed to archive external Codex thread."),
            cause,
          }),
      });
